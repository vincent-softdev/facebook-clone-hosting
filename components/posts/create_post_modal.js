...
import CustomModal from "../modal/custom_modal";
...
const CreatePostModal = ({ closeModal }) => {
    ...
    if (textAreaRef.current) {
      ...
      const lines = text.split("\n").reduce((totalLines, line) => {
        ...
        const charsPerLine = Math.floor(
          textAreaRef.current.clientWidth / (fontSize * 0.6)
  ...
  return (
    <CustomModal closeModal={closeModal} title="Create Post">
          ...
          <>
            <div
              ...
              style={{
                ...
                paddingBottom: "1rem",
                backgroundColor:
                  textareaColor != "#ffffff" && textLine < 4
                    ? textareaColor
                    : "white",
              }}
            >
              <textarea
                ...
                style={{
                  ...
                  height: `${
                    textLine * fontSize +
                    20 +
                    (textLine * fontSize > 270
                      ? textLine * fontSize > 300
                        ? 50
                        : 45
                      : 30)
                  }px`,
              ...
              <div
                className={`h-[40px] flex items-center ${
                  !(textLine < 4 && !imageActivated)
                    ? "justify-end"
                    : "justify-between"
                } `}
              >
                {textLine < 4 && !imageActivated && colorList && (
                  <button className="h-10" onClick={activateColorList}>
                    <IconImages.ColorfulTextIcon className="h-full cursor-pointer" />
                  </button>
                )}
                {!colorList && textLine < 4 && (
                  <div className="flex gap-2">
                    <button
                      onClick={activateColorList}
                      className="bg-[#c9ccd1] w-9 h-9 pr-1 rounded-lg flex justify-center items-center"
                    >
                      <ArrowBackIosNewIcon />
                    </button>
                    {postBackgroundColor.map((item, idx) => {
                      return (
                        <button
                          key={idx}
                          onClick={() => activateTextAreaColor(item.color)}
                          className="w-9 h-9 rounded-lg"
                          style={{
                            backgroundColor:
                              item.color == textareaColor &&
                              item.color == "#ffffff"
                                ? "#f2f4f7"
                                : item.color,
                            border:
              ...
              {imageActivated && (
                <div className="max-w-xl mb-4 relative">
                  ...
                  {imageToPost == null && (
                    <label className="flex justify-center w-full h-32 p-2 border-[1px] transition bg-white border-gray-300 rounded-md appearance-none cursor-pointer focus:outline-none">
                      <span className="flex items-center space-x-2 bg-gray-100 hover:bg-gray-200 w-full rounded-md">
                        <span className="flex flex-col items-center w-full">
                          ...
                          <span className="text-black font-medium text-[1.0625rem]">
                            Add Photos/Videos
                          </span>
            ...
            <div className="h-[60px] mx-4 flex border-gray-300 rounded-md border-[1px] px-4 font-semibold justify-between">
              <div className="h-full flex items-center">Add to your post</div>
              <div
                id="function-icon-cover"
